/**
 * A simple server that serves static files and forces authentication while using sessions
 *
 ...
 * the static files and will create a session after initial authentication.
 */
...
app.use(morgan('dev'));
// Set up the session information with a secret key
app.use(session({
  name: 'session-id',
 ...
 * with a status of 401. Creates a session on the client for tracking on the server.
 *
 * @see Error()
 * @see next()
 * @see Buffer()
 ...
 *         
 *      @type Object $headers The request headers sent to the server.
 *      @type Array $params URL parameters included with request.
 *      @type Object $body JSON object sent in the body of the request.
 *      @type Object $session Session object created by the server and sent to the client.
 * }
 * @params Object $res {
...
function auth(req, res, next) {
  ...
  // Check if incoming request has a cookie
  if(! req.session.user) {
    ...
    if(user == 'admin' && pass == 'password') {
      // Set up the cookie with the name user, value admin and signed
    ...
    // Check the signed cookie named user has a value of admin
    if(req.session.user === 'admin') {
      console.log('req.session: ', req.session);
      next();
    // Else throw and error with status 401
    } else {
      var err = new Error("You are not authenticated!");
      err.status = 401;
      next(err);
